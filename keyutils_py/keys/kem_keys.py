...
from keyutils_py.fips.frodokem import FrodoKEM
...
from keyutils_py.utils import oqs
...
class MLKEMPublicKey(PQKEMPublicKey):
    ...
    Encapsulation uses the ``cryptography`` native ML-KEM backend for
    ``ml-kem-768`` / ``ml-kem-1024``. ``ml-kem-512`` is not exposed by
    ``cryptography`` 48 and continues to dispatch to liboqs / FIPS-203.
    """

    _native_key: Optional[NativeMLKEMPub]

    def _initialize_key(self):
        """Initialize the ML-KEM public key."""
        self.ml_class = ML_KEM(self.name)
        self._native_key = None
        if self.name in NATIVE_MLKEM_NAMES:
            self._native_key = build_native_mlkem_pub(self.name, self._public_key_bytes)
        if oqs is not None:
    ...
    def encaps(self) -> Tuple[bytes, bytes]:
        ...
        if self._native_key is not None:
            return self._native_key.encapsulate()
...
class MLKEMPrivateKey(PQKEMPrivateKey):
    ...
    Backend selection mirrors :class:`MLDSAPrivateKey`:

    * If a seed is available and the algorithm is ``ml-kem-768`` /
      ``ml-kem-1024``, the key is held internally as a ``cryptography``-native
      ``MLKEM*PrivateKey``. Encapsulation / decapsulation and PKCS#8 ``seed``
      export use that backend directly. The expanded private bytes are
      lazy-derived via FIPS-203 only when the caller asks for ``RAW`` /
      ``SEED_AND_RAW`` export.
    * For ``ml-kem-512`` (not exposed by ``cryptography``) and for keys loaded
      from raw expanded bytes (no seed), behaviour is unchanged: liboqs when
      installed, otherwise the bundled FIPS-203 fallback.
    """

    _native_key: Optional[NativeMLKEMPriv]
    ...
    def _initialize_key(self):
        ...
        if self._seed is not None and len(self._seed) != 64:
            raise InvalidKeyData(f"Invalid seed length. Expected 64 bytes. Got: {len(self._seed)}")

        if self._seed is not None and self.name in NATIVE_MLKEM_NAMES:
            # Cryptography backend: hold native key. Whatever expanded bytes
            # were supplied stay as-is; the native key drives encaps / decaps.
            self._native_key = build_native_mlkem_priv(self.name, self._seed)
            if self._public_key_bytes is None:
                self._public_key_bytes = self._native_key.public_key().public_bytes_raw()

        elif self._seed is not None and self._private_key_bytes is None:
            # ML-KEM-512 (and any other non-native alg with seed-only inputs):
            # expand deterministically via FIPS-203.
    ...
    def private_bytes_raw(self) -> bytes:
        ...
        For cryptography-backed keys this expands the seed via FIPS-203 the
        first time it is requested and caches the result.
        """
        if self._private_key_bytes is None:
            if self._seed is None:
                raise ValueError("No seed available to derive ML-KEM private bytes.")
            d, z = self._seed[:32], self._seed[32:]
            _, self._private_key_bytes = self.ml_class.keygen_internal(d=d, z=z)
    ...
    def decaps(self, ct: bytes) -> bytes:
        ...
        """
        if self._native_key is not None:
            return self._native_key.decapsulate(ct)

        if oqs is not None:
            return super().decaps(ct)

        try:
            return self.ml_class.decaps_internal(dk=self.private_bytes_raw(), c=ct)
