...
from keyutils_py.fips import fips204, fips205
from keyutils_py.fips.fips204 import ML_DSA
from keyutils_py.fips.fips205 import SLH_DSA, integer_to_bytes
from keyutils_py.keys._crypto_backend import (
    NATIVE_MLDSA_NAMES,
    NativeMLDSAPriv,
    NativeMLDSAPub,
...
from keyutils_py.keys.abstract_pq import PQSignaturePrivateKey, PQSignaturePublicKey
from keyutils_py.oids import SLH_DSA_PRE_HASH_NAME_2_OID, compute_hash, sha_alg_name_to_oid
from keyutils_py.utils import encode_to_der, oqs
...
class MLDSAPublicKey(PQSignaturePublicKey):
    ...
    Verification uses the ``cryptography`` native ML-DSA backend by default and
    falls back to liboqs / the bundled FIPS-204 implementation only when a
    pre-hash variant is requested (``hash_alg`` is set or ``is_prehashed=True``).
    """

    _native_key: Optional[NativeMLDSAPub]

    def _initialize_key(self) -> None:
        """Initialize the ML-DSA public key."""
        self.ml_class = ML_DSA(self.name)
        self._native_key = None
        if self.name in NATIVE_MLDSA_NAMES:
            self._native_key = build_native_mldsa_pub(self.name, self._public_key_bytes)
        if oqs is not None:
    ...
    def verify(
        ...
        if hash_alg is None and not is_prehashed and self._native_key is not None:
            self._native_key.verify(signature, data, ctx if ctx else None)
            return None
        ...
        if hash_alg is None:
            ...
            oid = encode_to_der(sha_alg_name_to_oid(hash_alg))
...
class MLDSAPrivateKey(PQSignaturePrivateKey):
    ...
    Backend selection happens once in :meth:`_initialize_key`:

    * If a seed is available (fresh generate, ``seed=`` constructor arg, or a
      seed-bearing PKCS#8 ``CHOICE`` payload) the key is held internally as a
      ``cryptography``-native :class:`~cryptography.hazmat.primitives.asymmetric.mldsa.MLDSA*PrivateKey`.
      Sign / verify and PKCS#8 ``seed`` export use that backend directly. The
      raw expanded private bytes are computed lazily via the bundled FIPS-204
      reference implementation only when a caller asks for ``RAW`` or
      ``SEED_AND_RAW`` export.
    * If only the expanded private bytes are available (no seed) the key falls
      back to liboqs / the bundled FIPS-204 path. Pre-hashed sign / verify
      always uses that path because the cryptography library has no API for it.
    """

    _native_key: Optional[NativeMLDSAPriv]
    ...
    def _initialize_key(self) -> None:
        ...
        if self._seed is None and self._private_key_bytes is None and self._public_key_bytes is None:
            self._seed = os.urandom(32)

        if self._seed is not None and self.name in NATIVE_MLDSA_NAMES:
            # Cryptography backend: hold the native key. Whatever expanded
            # bytes the caller supplied are kept as-is; the native key drives
            # subsequent operations regardless.
            self._native_key = build_native_mldsa_priv(self.name, self._seed)
            if self._public_key_bytes is None:
                self._public_key_bytes = self._native_key.public_key().public_bytes_raw()

        elif self._private_key_bytes is None and self._public_key_bytes is None:
            # No seed and no expanded bytes (only possible on a non-native alg
            # that hits this class — kept for parity with the original logic).
            raise ValueError(
        ...
        if oqs is not None and self._native_key is None:
            self._sig_method = oqs.Signature(self._other_name, secret_key=self._private_key_bytes)

    def private_bytes_raw(self) -> bytes:
        """Return the raw expanded private-key bytes.

        For cryptography-backed keys this expands the seed deterministically via
        FIPS-204 the first time it is requested and caches the result.
        """
        if self._private_key_bytes is None:
            if self._seed is None:
                raise ValueError("No seed available to derive ML-DSA private bytes.")
            _, self._private_key_bytes = self.ml_class.keygen_internal(xi=self._seed)
    ...
    def sign(
        ...
        if hash_alg is None and not is_prehashed and self._native_key is not None:
            return self._native_key.sign(data, ctx if ctx else None)
        ...
        if hash_alg is None:
            ...
            oid = encode_to_der(sha_alg_name_to_oid(hash_alg))  # type: ignore
            ...
            sig = ml_.sign_internal(self.private_bytes_raw(), mp, os.urandom(32))
