...
from keyutils_py.keys.abstract_stateful_hash_sig import PQHashStatefulSigPrivateKey, PQHashStatefulSigPublicKey
...
class XMSSPublicKey(PQHashStatefulSigPublicKey):
    ...
    def key_bit_security(self) -> int:
        """Return the PQ bit security of the XMSS public key.

        Approximate bit security based on the hash output size; halved for
        Grover's algorithm per RFC 8391 §5.
        """
...
class XMSSPrivateKey(PQHashStatefulSigPrivateKey):
    ...
    def from_private_bytes(cls, data: bytes):
        ...
        # The BDS traversal cache follows; without it, the key size would be significantly larger.
...
class XMSSMTPublicKey(PQHashStatefulSigPublicKey):
    ...
    def key_bit_security(self) -> int:
        ...
        Approximate bit security; equals ``n * 4`` where ``n`` is the hash
...
class HSSPublicKey(PQHashStatefulSigPublicKey):
    ...
    @property
    def key_bit_security(self) -> int:
        """Return the PQ bit security of this HSS key.

        Derived from the LMS hash output size ``n``: ``n=24`` → 96 bits,
        otherwise 128 bits.
        """
        # TODO fix for multiple levels with different parameters.
        n = self._details["n"]
