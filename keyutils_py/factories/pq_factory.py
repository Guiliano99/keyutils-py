...
from keyutils_py.data_objects import (
...
from keyutils_py.exceptions import BadAlg, BadAsn1Data, InvalidKeyData, MismatchingKey
from keyutils_py.factories.abstract_factory import AbstractKeyFactory
from keyutils_py.keys.abstract_pq import PQKEMPrivateKey, PQKEMPublicKey, PQSignaturePrivateKey
...
from keyutils_py.keys.sig_keys import (
    FalconPrivateKey,
    FalconPublicKey,
    MLDSAPrivateKey,
    MLDSAPublicKey,
...
class PQKeyFactory(AbstractKeyFactory):
    ...
    @staticmethod
    def _load_from_one_asym_key_bytes(
        name: str,
        private_bytes: bytes,
        public_bytes: Optional[bytes],
    ) -> PQPrivateKey:
        ...
        if name.startswith("ml-dsa-"):
            cls_name: Type[PQPrivateKey] = MLDSAPrivateKey
        elif name.startswith("slh-dsa"):
            cls_name = SLHDSAPrivateKey
        elif name.startswith("ml-kem"):
            cls_name = MLKEMPrivateKey
        else:
            raise NotImplementedError(f"Unimplemented algorithm: {name}")

        return PQKeyFactory._load_and_validate(cls_name, name, private_bytes, public_bytes)
    ...
    def load_ml_private_key_from_one_asym_key(
        ...
        data, rest = try_decode_pyasn1(private_bytes, structure())
    ...
    def may_be_pq_alg(algorithm: str) -> bool:
        ...
        """
        return PQKeyFactory._starts_with_any(algorithm, PQKeyFactory._prefixes)
    ...
    def _load_pq_private_support_pub_derive(
        ...
        private_key_type: Type[PQKEMPrivateKey]
    ...
    def _load_private_kem_key(
        ...
        """
        if PQKeyFactory._starts_with_any(name, ["ml-kem", "sntrup761", "frodokem"]):
    ...
    def _load_private_sig_key(
        ...
        """
        if PQKeyFactory._starts_with_any(name, ["ml-dsa", "slh-dsa"]):
    ...
    def _load_private_key_from_pkcs8(
        ...
        if alg_name in PQ_KEM_NAME_2_OID:
            return cls._load_private_kem_key(
                name=alg_name,
                private_key_bytes=private_key_bytes,
                public_key_bytes=public_key_bytes,
            )
        if alg_name in PQ_SIG_NAME_2_OID:
            return cls._load_private_sig_key(
                name=alg_name,
                private_key_bytes=private_key_bytes,
                public_key_bytes=public_key_bytes,
            )
    ...
    def from_one_asym_key(
        ...
        if isinstance(one_asym_key, bytes):
            one_asym_key = try_decode_pyasn1(one_asym_key, rfc5958.OneAsymmetricKey())[0]  # type: ignore[assignment]
        ...
        # ML-DSA and ML-KEM MUST use the standardised Choice (seed / expandedKey
        # / both) format defined in the IETF drafts. The pre-Choice raw-bytes
        # form is deprecated and no longer accepted: any decode failure here
        # surfaces as InvalidKeyData rather than falling back to raw bytes.
        if name.startswith("ml-kem-") or name.startswith("ml-dsa-"):
            try:
                ...
                raise InvalidKeyData(
                    f"Invalid key data for {name}: the deprecated raw-bytes format is no longer "
                    f"supported. Re-encode the key using the standard Choice structure "
                    f"(seed / expandedKey / both)."
                ) from e

        if name.startswith("slh-dsa"):
            return PQKeyFactory._load_from_one_asym_key_bytes(name, private_bytes, public_bytes)

        return PQKeyFactory._load_private_key_from_pkcs8(
    ...
    def _prepare_ml_private_key(
        ...
        return encode_to_der(structure)
    ...
    def _may_get_pub_key(
        ...
        if not isinstance(private_key, (MLDSAPrivateKey, MLKEMPrivateKey, SLHDSAPrivateKey)):
            if version == 0 or (include_pub_key is False and not unsafe):
    ...
    def save_private_key_one_asym_key(
        ...
        der_data = encode_to_der(one_asym_key)
        return der_data
