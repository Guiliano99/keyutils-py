...
class FrodoKEM(object):
    ...
    def __print_intermediate_value(self, name, value):
        ...
        if not (self.print_intermediate_values):
            return None
        ...
        for i in range(self.mbar):
            ...
            for j in range(self.nbar):
                ...
                for l in range(self.B):
                    if kbits[(i * self.nbar + j) * self.B + l]:
                        tmp += 2**l
    ...
    def kem_keygen(self, seed: Optional[bytes] = None):
    ...
    def kem_encaps(self, pk):
        ...
        rbytes = self.shake(
            bytes(b"\x96") + seedSE, (2 * self.mbar * self.n + self.mbar * self.mbar) * self.len_chi_bytes
        )
        ...
        Eprimeprime = self.sample_matrix(
            r[2 * self.mbar * self.n : 2 * self.mbar * self.n + self.mbar * self.nbar], self.mbar, self.nbar
        )
    ...
    def kem_decaps(self, sk, ct):
        ...
        rbytes = self.shake(
            bytes(b"\x96") + seedSEprime, (2 * self.mbar * self.n + self.mbar * self.mbar) * self.len_chi_bytes
        )
        r = [struct.unpack_from("<H", rbytes, 2 * i)[0] for i in range(2 * self.mbar * self.n + self.mbar * self.nbar)]
        self.__print_intermediate_value("r", r)
