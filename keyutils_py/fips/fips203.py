...
from keyutils_py.fips._fips_utils import XOFHash, _compute_hash, _compute_shake

#   Table 2. Approved parameter sets for ML-KEM
#   (k, eta1, eta2, du, dv)

ML_KEM_PARAM = {"ml-kem-512": (2, 3, 2, 10, 4), "ml-kem-768": (3, 2, 2, 10, 4), "ml-kem-1024": (4, 2, 2, 11, 5)}
...
class ML_KEM:
    ...
    def multiply_ntts(self, f, g):
        ...
        for ii in range(0, 256, 2):
            h += self.best_case_multiply(f[ii], f[ii + 1], g[ii], g[ii + 1], ML_KEM_ZETA_MUL[ii // 2])
        return h
    ...
    def k_pke_decrypt(self, dk_pke, c):
        ...
        up = [
            self.decompress(self.du, self.byte_decode(self.du, c1[32 * self.du * i : 32 * self.du * (i + 1)]))
            for i in range(self.k)
        ]
        # print('# u:', up)
    ...
    def keygen_internal(self, d, z, param: Optional[str] = None) -> Tuple[bytes, bytes]:
        """Generate the ML-KEM private key.
