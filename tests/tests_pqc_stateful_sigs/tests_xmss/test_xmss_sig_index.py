...
from keyutils_py.keys.stateful_sig_keys import XMSSPublicKey, XMSSPrivateKey
from keyutils_py import load_private_key_from_file
from tests.test_helpers import get_all_xmss_xmssmt_keys
...
class TestXMSSIndexSignature(unittest.TestCase):
    ...
    def test_index_signature_after_deserialization(self):
        """
        GIVEN an XMSS stateful signature instance,
        WHEN generating a keypair, signing a message, and then restoring the key from its used-key export,
        THEN the new signature produced by the restored key should have index 1.
        """
        with StatefulSignature(self.algorithm) as sig:
            public_key = sig.generate_keypair()
    ...
    def test_index_signature_comp(self):
        """
        GIVEN an XMSS-SHA2_10_256 key with 1024 possible signatures,
        WHEN signing the same message max_sig_size times in sequence,
        THEN each signature's leaf index should equal the signing iteration count.
        """
    ...
    def test_index_signature_comp_shake(self):
        """
        GIVEN an XMSS-SHAKE_10_256 key with 1024 possible signatures,
        WHEN signing the same message max_sig_size times in sequence,
        THEN each signature's leaf index should equal the signing iteration count.
        """
    ...
    def test_index_zero_and_one_signature_all(self):
        """
        GIVEN all bundled XMSS keys (XMSSMT keys are excluded),
        WHEN signing two different messages in sequence with each key,
        THEN the first signature should have leaf index 0 and the second should have leaf index 1.
        """
