...
class TestXMSSStatefulSignature(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.xmss_algorithm = "XMSS-SHA2_10_256"
    ...
    def test_xmss_saved_used_keys(self):
        ...
        with StatefulSignature(self.xmss_algorithm) as sig:
            public_key = sig.generate_keypair()
            self.assertIsNotNone(public_key, "Public key generation failed")
            message = os.urandom(32)
            signature = sig.sign(message)
            self.assertTrue(sig.verify(message, signature, public_key), "Signature verification failed")
            signature2 = sig.sign(b"Hello, XMSS!")
            self.assertTrue(sig.verify(b"Hello, XMSS!", signature2, public_key), "Second signature verification failed")
    ...
    def test_xmss_key_generation_signing_serialization(self):
        ...
        with StatefulSignature(self.xmss_algorithm) as sig:
            ...
            # Sign two messages
            signature = sig.sign(message)
            self.assertTrue(sig.verify(message, signature, public_key), "First signature verification failed")

            second_sig = sig.sign(message2)
            self.assertTrue(sig.verify(message2, second_sig, public_key), "Second signature verification failed")
            self.assertFalse(sig.verify(message2, signature, public_key), "Old signature should not verify new message")
        ...
        sig2 = StatefulSignature(self.xmss_algorithm, secret_key=out[0])
        ...
        self.assertTrue(
            sig2.verify(new_message, new_signature, public_key), "Signature verification after deserialization failed"
        )
