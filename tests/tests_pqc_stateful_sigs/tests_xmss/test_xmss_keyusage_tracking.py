...
class TestXMSSKeyUsageTracking(unittest.TestCase):
    ...
    def test_total_and_remaining_signatures_tracking(self):
        ...
        with StatefulSignature(self.algorithm) as sig:
            ...
            sig.sign(self.message)
            sig.sign(self.message2)

            remain_after = sig.sigs_remaining()
            self.assertEqual(remain_after, total_before - 2, "Remaining signatures did not decrease by 2")
        ...
        new_sig = sig2.sign(self.message3)
        self.assertTrue(sig2.verify(self.message3, new_sig, public_key), "Signature after deserialization invalid")
